...
# script for the K16 in APC emulation mode
...
class K16(ControlSurface):
    __doc__ = " Script for K16 in APC emulation mode "

    _active_instances = []
    def _combine_active_instances():
        track_offset = 0
        scene_offset = 0
        for instance in K16._active_instances:
    ...
    def _do_combine(self):
        if self not in K16._active_instances:
            K16._active_instances.append(self)
            K16._combine_active_instances()


    def _do_uncombine(self):
        if ((self in K16._active_instances) and K16._active_instances.remove(self)):
